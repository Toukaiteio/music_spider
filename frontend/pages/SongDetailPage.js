...
class SongDetailPage {
    ...
    onLoad(mainContentElement, subPageId, appState, managers) {
        ...
        const descriptionEl = mainContentElement.querySelector("#detail-description");
        ...
        const playButtonEl = mainContentElement.querySelector(".detail-play-button");
        const addToCollectionButtonEl = mainContentElement.querySelector(".detail-add-to-collection-button");
        ...
        const backButton = mainContentElement.querySelector("#song-detail-back-button");
        if (backButton && managers.navigationManager) {
            ...
            newBackButton.addEventListener("click", managers.navigationManager.navigateBack);
        }

        // Lyrics and Upload Lyrics button logic
        const lyricsDisplayArea = mainContentElement.querySelector("#lyrics-display-area");
        const uploadLyricsButton = mainContentElement.querySelector("#upload-lyrics-button");
        ...
        if (lyricsDisplayArea && uploadLyricsButton) {
            if (track.lyrics && typeof track.lyrics === 'string' && track.lyrics.trim() !== '') {
                ...
                if (typeof parseLRC === "function") {
                    ...
                    lyricsDisplayArea.innerHTML = `<pre>${track.lyrics.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</pre>`;
                ...
                if (parsedLyricsData.length > 0) {
                    ...
                    const drawLyrics = () => {
                        ...
                        for (let i = start; i < end; i++) {
                            const y = centerY + (i - currentLineIndex - (scrollOffset - currentLineIndex)) * lineHeight;
                    ...
                    const animateLyrics = () => {
                        if (!canvas || !ctx) { // check if canvas is still in DOM
                    ...
                    const resizeCanvas = () => {
                        ...
                        if (ctx) { // Ctx might not exist if getContext fails
                ...
                uploadLyricsButton.onclick = () => {
                    ...
                    if(managers.navigationManager) {
                        managers.navigationManager.navigateTo("update-track", `Update ${track.title || "Track"}`, `#update-track/${songId}`, false, songId);
                    }
                };
            }
        }
        ...
        // Event Handlers
        const detailPlayButton = mainContentElement.querySelector(".detail-play-button");
        const detailAddToCollectionButton = mainContentElement.querySelector(".detail-add-to-collection-button");
        const detailUpdateButton = mainContentElement.querySelector(".detail-update-button");
                ...
                if (songId) {
                    ...
                    if (trackInfoString) {
                        try {
                            ...
                            if(managers.uiManager) managers.uiManager.showToast("Error processing track data.", "error");
                        }
                    } else if(managers.uiManager) {
                        managers.uiManager.showToast("Cannot add to collection: Missing track data.", "error");
                    }
                }
            });
        }

        if (detailUpdateButton && managers.navigationManager && managers.uiManager) {
            detailUpdateButton.addEventListener('click', () => {
                if (appState.currentSongDetail) {
                    ...
                    if (musicId) {
                        ...
                        managers.uiManager.showToast("Cannot update: Music ID is missing.", "error");
                    }
                } else { 
                    managers.uiManager.showToast("Cannot update: No song details available.", "error");
                }
            });
        }
    }
