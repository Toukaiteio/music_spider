...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const favoriteManager = new FavoriteManager({ appState: window.appState, uiManager: UIManager }); // Initialize FM
  ...
  mainContent.addEventListener('change', function(event) {
    if (event.target.id === 'upload-cover-file-input') {
        UIManager.handleCoverFileSelect(event.target, "upload");
    } else if (event.target.id === 'update-cover-file-input') {
        UIManager.handleCoverFileSelect(event.target, "update");
    }
  });
  ...
  mainContent.addEventListener("click", function (event) {
    ...
    if (playButton) {
      ...
      if (songCard && navigationManager.getCurrentPageId() !== "search-results") { // Avoid nav on search results page cards
        const trackInfoString =
          songCard.querySelector(".play-on-card-button")?.dataset.trackInfo ||
          songCard.querySelector(".add-to-collection-button")?.dataset.trackInfo ||
      ...
      const trackInfoString = addToDownloadQueueButton.dataset.trackInfo;
        if (trackInfoString) {
            try {
                ...
                UIManager.showToast("Could not add track to queue: Invalid track data.", "error");
            }
        }
    }
  ...
  if (playerCoverArea) {
    playerCoverArea.addEventListener("click", () => {
      ...
      if (currentTrack) { // Ensure currentTrack exists
  ...
  window.test.renderTaskQueue = UIManager.renderTaskQueue ? UIManager.renderTaskQueue.bind(UIManager) : () => console.warn("UIManager.renderTaskQueue not found.");
  window.test.updateMainTaskQueueIcon = UIManager.updateMainTaskQueueIcon ? UIManager.updateMainTaskQueueIcon.bind(UIManager) : () => console.warn("UIManager.updateMainTaskQueueIcon not found.");
